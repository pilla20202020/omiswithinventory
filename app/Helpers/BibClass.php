...
?>
    ...
    public static function controllerContent($tableName, $directoryName)
    {
           ...
           public function index()
            {
                ...
                return view(\"omis.$viewName.index\", compact('data'));
            }
        ...
        if (!file_exists($filename)) {
            if (!file_exists('../app/Http/Controllers/' . $controllerDirectory)) {
                mkdir('../app/Http/Controllers/' . $controllerDirectory, 0777, true);
            }
            $file = fopen("../app/Http/Controllers/" . $controllerDirectory . $filename, 'w');
            fwrite($file, $contentString);
            fclose($file);
        }
        return $contentString;
    ...
    public static function migrationContent($tableName)
    {
        ...
        foreach ($tableFields as $tableField) {
            ...
            switch ($Type) {
                ...
                case 'varchar(255)':
                    $contentString .= "\$table->string(\"$tableField->Field\");\n";
                    break;

                case 'datetime':
                    $contentString .= "\$table->datetime(\"$tableField->Field\");\n";
                    break;

                case 'text':
                    $contentString .= "\$table->text(\"$tableField->Field\");\n";
                    break;
        ...
        $file = fopen("../database/migrations/create_" . $tableName . "_table.php", 'w');
        fwrite($file, $contentString);
        fclose($file);
        return $contentString;
        //        return $tableFields;
...
@endpush
        ...
        $path = "../resources/views/omis/$folder/";
    ...
    public static function ajaxListContent($TableName, $directoryName)
    {
        ...
        $listContent = '<table class="datatable-init table" data-nk-container="table-responsive">
        <thead class="table-light">
            <tr>
            <th class="tb-col"><span class="overline-title">S.N.</span></th>' . PHP_EOL;
        ...
        $path = "../resources/views/omis/ajax/$folder/";
        $file = $path . "index.blade.php";
        if (!file_exists("$path")) {
            mkdir("$path", 0777, true);
        }
    ...
    public static function editContent($TableName, $directoryName)
    {
        ...
        $folder .= str_replace("tbl_", "", $TableName);
        $Table_pk = str_replace("tbl_", "", $TableName) . "_id";
        $editContent = "<form method=\"POST\" action=\"{{route('$routeName.update')}}\" enctype=\"multipart/form-data\">\n
        @csrf \n
            @method('PUT')\n";
        foreach ($TableCols as $TableCol) :
            $TableCol = $TableCol->Field;
            $TableColLabel = ucwords(str_replace("_", " ", $TableCol));
            $editContent .= "{{createText(\"$TableCol\",\"$TableCol\",\"$TableColLabel\")}}\n";
        endforeach;
        $editContent .= "<?php createButton(\"btn-primary\",\"\",\"Submit\"); ?>\n";
        $editContent .=  " </form></div></div></div>\n@endsection";
        ...
        // if (!file_exists("../resources/views/omis/$folder/edit.blade.php")) {
        //     $file = fopen("../resources/views/omis/$folder/edit.blade.php", 'w');
        //     fwrite($file, $editContent);
        //     fclose($file);
        // }
        return $editContent;
    ...
    public static function ajaxEditContent($TableName, $directoryName)
    {
        ...
        $Table_pk = str_replace("tbl_", "", $TableName) . "_id";
        $title = ucwords(str_replace("tbl_", "", $TableName));
        $editContent = "<form method=\"POST\" action=\"{{route('$routeName.update',[\$data->$Table_pk])}}\" enctype=\"multipart/form-data\">\n @csrf \n @method('PUT')\n";
        $editContent .= '<div class="row">';
        foreach ($TableCols as $key => $TableCol) :
            $TableCol = $TableCol->Field;
            if ($key == 0 || $TableCol == 'createdOn' || $TableCol == 'createdBy' || $TableCol == 'updatedBy' || $TableCol == 'created_at' || $TableCol == 'updated_at')
